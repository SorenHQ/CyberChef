...
import { hexbin as hexbinn } from "d3-hexbin";
...
class HexDensityChart extends Operation {
    ...
    run(input, args) {
        ...
        const hexbin = hexbinn()
            .radius(packRadius)
            .extent([0, 0], [width, height]);

        const hexPoints = hexbinn(values),
            maxCount = Math.max(...hexPoints.map((b) => b.length));

        const xExtent = extent(hexPoints, (d) => d.x),
            yExtent = extent(hexPoints, (d) => d.y);
        xExtent[0] -= 2 * packRadius;
        xExtent[1] += 3 * packRadius;
        yExtent[0] -= 2 * packRadius;
        yExtent[1] += 2 * packRadius;

        const xAxis = scaleLinear().domain(xExtent).range([0, width]);
        const yAxis = scaleLinear().domain(yExtent).range([height, 0]);

        const colour = scaleSequential(
            interpolateLab(minColour, maxColour)
        ).domain([0, maxCount]);
        ...
        marginedSpace
            ...
            .call(axisLeft(yAxis).tickSizeOuter(-width));
        ...
        marginedSpace
            ...
            .call(axisBottom(xAxis).tickSizeOuter(-height));
    ...
    getEmptyHexagons(centres, radius) {
        const emptyCentres = [],
            boundingRect = [
                extent(centres, (d) => d.x),
                extent(centres, (d) => d.y),
            ],
