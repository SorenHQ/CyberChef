...
import { scaleLinear, select, extent, axisLeft, scalePoint, axisTop } from "d3";
...
class SeriesChart extends Operation {
    ...
    run(input, args) {
        ...
        const xAxis = scalePoint().domain(xValues).range([0, seriesWidth]);

        svg.append("g")
            .attr("class", "axis axis--x")
            .attr("transform", `translate(${seriesLabelWidth}, ${xAxisHeight})`)
            .call(
                axisTop(xAxis).tickValues(
                    xValues.filter((x, i) => {
                        return (
                            [
                                0,
        ...
        series.forEach((serie, seriesIndex) => {
            const yExtent = extent(Object.values(serie.data)),
                yAxis = scaleLinear().domain(yExtent).range([seriesHeight, 0]);
